  ...
  async getAllSoh(): Promise<Component[]> {
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        // Map backend's 'part_code' to frontend's expected 'partCode' for MRP compatibility
    ...
    async getSohByPartCode(partCode: string): Promise<Component | null> {
      try {
        ...
        if (response.success && response.data && response.data.length > 0) {
          console.log(`✅ Found SOH record for part code: ${partCode}`);
          return response.data[0];
        }
        
        return null;
      } catch (error) {
        console.error('❌ Error fetching SOH by part code:', error);
        return null;
      }
    }
...
export const getSohByPartCode = (partCode: string) => inventoryService.getSohByPartCode(partCode);
